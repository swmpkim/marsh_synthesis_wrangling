...
```{r}
path_start <- here::here("submitted_data", "data", "NOC", "Masonboro_Island_component")
problem_path_out <- here::here("wrangled_data", "combined_with_issues")
path_out <- here::here("wrangled_data", "NOC_MI")
...
files_matching <- str_which(files_all, "NOC_MI_\\d{4}\\.xlsx")
...
# # density has both double and character: "TNTC" in 2020 file
# purrr::map(dat_in, ~unique(.x$Density))
# # so does Cover: must be cell formatting in Excel
...
```{r}
write.csv(dat_cdmo, here::here(problem_path_out, "NOC-MI_issues.csv"), 
          row.names= FALSE,
          na = "")
```
