...
dens_ht <- dens_ht %>% 
    mutate(Date_excel = as.numeric(case_when(!str_detect(Date, "/") ~ Date,
           ...
           Site = case_when(Site == "DP" ~ "DM",
                            TRUE ~ Site)) %>% 
    rename(SiteID = Site) %>% 
    select(-Date_excel, -Date_mdy, -Year)
```
...
dens <- dens_ht %>% 
    select(ID:Date, "Notes" = NOTES, all_of(dens_names)) %>% 
...
hts <- dens_ht %>% 
    select(ID:Date, "Notes" = NOTES, all_of(ht_names))%>% 
    pivot_longer(-(ID:Notes), 
                 names_to = c("code", "param"), 
                 names_sep = " ",
                 values_to = "Height") %>% 
    filter(!is.na(Height)) %>%
    group_by(SiteID, TransectID, PlotID, Date, code) %>% 
...
dat_all_problem_version <- dat_all %>% 
