...
colsToFix <- which(names(dat_cdmo) %in% c("SSAM-1", "Lat", "Long", "Distance", "Orthometric Height", "Height Relative to MLLW"))
...
NMST_dupes <- covr_sheet_out %>% 
    ...
    janitor::get_dupes(Date, SiteID, TransectID, PlotID) %>% 
    relocate(NMSTrownum) %>% 
    select(NMSTrownum:`Height Relative to MLLW`)
