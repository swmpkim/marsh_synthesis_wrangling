...
qaqc_to_join <- dat_cdmo %>% 
    filter(is.na(Subplot)) %>% 
    select(Reserve:Species, QAQC) %>% 
    pivot_wider(names_from = Species, values_from = QAQC, names_prefix = "F_")
```
...
ht_wide <- ht_sub %>% 
    select(-QAQC, -Subplot) %>% 
    pivot_wider(names_from = Species, values_from = `Maximum Canopy Height`)
...
dens_to_join <- dens_means %>% 
    pivot_wider(names_from = Species, values_from = Density, names_prefix = "Density_")
