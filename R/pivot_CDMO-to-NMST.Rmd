...
# reserve <- "LKS"  # Lemna dupe issue fixed; completed 6/28/22
...
```{r}
hts <- dat %>% 
    select(all_of(id_names), Species, Height = "Maximum Canopy Height") %>% 
    filter(!is.na(Height)) %>% 
    pivot_wider(names_from = Species,
                values_from = Height,
...
# get the 'Density' prefix out of the names
names(hts_sheet) <- str_replace(names(hts_sheet), "^(Maximum Canopy Height_)", "")
```
