...
elevs <- dat %>% 
    select(SiteID, TransectID, PlotID, "SSAM-1",
           Distance, "Orthometric_Height",
           "Height_Relative_to_MLLW") %>% 
    distinct() %>% 
    group_by(SiteID, TransectID, PlotID) %>% 
    summarize(Orthometric_Height = mean(Orthometric_Height, na.rm = TRUE),
              Height_Relative_to_MLLW = mean(Height_Relative_to_MLLW, na.rm = TRUE)) %>%
    arrange(SiteID, TransectID, PlotID)
